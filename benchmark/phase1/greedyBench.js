...
const CFG = { rep: (fa) => { const f = fitClass(fa, negPool, Mglob, { rho: RHO, delta: DBAND }); return { fires: f.head.fires, neg: f.neg, Qs: f.G.map((g) => g.Qbits), k: f.G.length }; },
  grd: (fa) => { const g = fitClassGreedy(fa, negPool, Mglob, { delta: DBAND, peel: true, candidates: "pPlus" }); return { fires: g.fires, neg: g.neg, Qs: g.parts.map((p) => p.Qs), k: g.parts.length }; },
  grdU: (fa) => { const g = fitClassGreedy(fa, negPool, Mglob, { delta: DBAND, peel: true, candidates: "union" }); return { fires: g.fires, neg: g.neg, Qs: g.parts.map((p) => p.Qs), k: g.parts.length }; } };
const KEYS = ["rep", "grd", "grdU"];
const w = process.stdout;
w.write(`# greedy(peel) vs replicator | canon=${TARGETS.length} | 4-fold | held-rec/confFP/K/jac/build-ms\n`);
w.write(`  word          |A|    replicator (fitClass)        greedy pPlus (peel)           greedy ⋁A-union (peel)\n`);
const agg = Object.fromEntries(KEYS.map((k) => [k, { r: 0, f: 0, k: 0, j: 0, ms: 0 }])); agg.n = 0;
const rows = [];
for (const word of TARGETS) {
  ...
  for (let fold = 0; fold < 4; fold++) {
    ...
    for (const key of KEYS) {
      ...
      acc[key][2] += m.k; acc[key][3] += intraJac(m.Qs); acc[key][4] += dt;
    }
  }
  const row = { word, A: Aw.length }; const cells = KEYS.map((key) => {
    const a = acc[key].map((v) => v / 4);
    agg[key].r += a[0] * 100; agg[key].f += a[1] * 100; agg[key].k += a[2]; agg[key].j += a[3]; agg[key].ms += a[4];
    row[key] = { R: +(a[0] * 100).toFixed(1), F: +(a[1] * 100).toFixed(1), K: +a[2].toFixed(1) };
    return `${(a[0] * 100).toFixed(0)}/${(a[1] * 100).toFixed(0)}/${a[2].toFixed(0)}/${a[3].toFixed(2)}/${a[4].toFixed(0)}`.padEnd(29);
  });
  agg.n++; rows.push(row);
  w.write(`  ${word.padEnd(12)}${String(Aw.length).padStart(6)}   ${cells.join("")}\n`);
}
const M = (o) => `${(o.r / agg.n).toFixed(0)}/${(o.f / agg.n).toFixed(0)}/${(o.k / agg.n).toFixed(1)}/${(o.j / agg.n).toFixed(2)}/${(o.ms / agg.n).toFixed(0)}`;
w.write(`  ${"MEAN".padEnd(12)}${"".padStart(6)}   ${KEYS.map((k) => M(agg[k]).padEnd(29)).join("")}\n`);
w.write(`# speedup grd ${(agg.rep.ms / agg.grd.ms).toFixed(2)}× · grdU ${(agg.rep.ms / agg.grdU.ms).toFixed(2)}× | JSON ${JSON.stringify(rows)}\n`);
